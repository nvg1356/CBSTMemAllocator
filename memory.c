...
static BSTNode* split_free(size_t desired_size, BSTNode* big_block) {
	BSTNode* new_free = align(HEADER_SIZE + big_block->pPayload);
	new_free->size = (void*) big_block + big_block->size - (void*) new_free;
	big_block->size = desired_size;
	return new_free;
}	
...
void* custom_alloc(size_t desired_size) {
	...
	if (node == NULL) {
		void* pNewUsage = align(pCurrent + HEADER_SIZE + desired_size); 
		if (pNewUsage > pBrk) {
...
void* custom_realloc(void* payloadptr, size_t new_size) {
	...
	void* newpayloadptr = custom_alloc(new_size);
...
void custom_free(void* payloadptr) { 
	...
	BSTNode* adj_node = ((void*) freed_node) + HEADER_SIZE + freed_node->size;
	while (true) {
